...
function MessageHeader({
  ...
  const sharePayload = useMemo(() => {
    const pageUrl = typeof window !== "undefined" ? window.location.href : "https://rolling.com";
    const recipientTitle = recipient?.name ? `To. ${recipient.name}` : "To. 이름 없는 대상";
    const writers = messageCount ?? 0;
    const description =
      writers > 0 ? `${writers}명이 작성해준 롤링페이퍼` : "롤링페이퍼를 만들어 보세요!";
    const shareImage =
      ...
      <div className="flex items-center justify-center w-full bg-white">
        <div className="flex items-center justify-between w-full max-w-[1200px] px-6 h-[68px]">
          ...
          <div className="flex items-center gap-[28px] relative">
            ...
            {/* 이모지 표시 + 화살표 */}
            <div className="flex items-center gap-2 min-w-[236px] w-[280px] max-xt:w-auto justify-end">
